...
async fn peek_bucket(mut s: Pin<&mut Peekable<BucketStream>>) -> Result<Option<u64>> {
    let is_err = match s.as_mut().peek().await {
        None => return Ok(None),
        Some(Ok((b, _))) => return Ok(Some(*b)),
        Some(Err(_)) => true,
    };
    if is_err {
        // Borrow from peek() is dropped; safe to reborrow and consume the Err.
        let item = s
            .as_mut()
            .next()
            .await
            .expect("peek reported Err, next must yield it");
        item?;
    }
    unreachable!("is_err == true implies the `?` above returned Err")
}
