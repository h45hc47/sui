...
mod checked {
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        let phase_result: Result<Mode::ExecutionResults, Mode::Error> = gas_charger
            ...
            .and_then(|v| {
                gas_charger.finalize_storage(temporary_store)?;
                Ok(v)
            })
            .and_then(|v| {
        ...
        if phase_result.is_err() {
            reset_charges(gas_charger, temporary_store);
        }
    ...
    fn reset_charges(gas_charger: &mut GasCharger, temporary_store: &mut TemporaryStore<'_>) {
        ...
        if dont_charge_budget_on_storage_oog(gas_charger.gas_model_version())
            && gas_charger.finalize_storage(temporary_store).is_ok()
        {
            return;
        }
